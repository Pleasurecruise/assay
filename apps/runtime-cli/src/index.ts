...
const apiKey =
  Bun.env.ASSAY_MODEL_API_KEY ??
  (provider === "deepseek" ? Bun.env.DEEPSEEK_API_KEY : undefined);
...
if (!apiKey) {
  throw new Error(
    "Missing ASSAY_MODEL_API_KEY (DEEPSEEK_API_KEY is also accepted for DeepSeek)",
  );
}
...
if (!model) {
  throw new Error(
    `Model "${provider}/${modelId}" is not present in the bundled oh-my-pi catalog`,
  );
}
