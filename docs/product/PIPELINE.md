...
总预算 20 分钟（赛道硬约束），分配如下：

| 阶段 | 预算 | 内容 | 并行度 |
| --- | --- | --- | --- |
| ① 任务解析 | 1 min | 识别策略 → 检查计划 → 回测次数分配 | — |
| ② 五项检查 | 10-12 min | 各检查独立取数、跑变体、出结构化结果 | 5 路并行 |
| ③ 交叉验证 | 3-4 min | 矛盾检测 → 追加实验（≤2 组）→ 结论合成 | 追加实验可并行 |
...
- 五项检查一项不少（数据拿不到 → 该项标"证据不足"，不是跳过）
- Moiré 矛盾检测（追加实验可从 2 组降到 1 组，不降到 0）
...
| 总时间逼近 18 min | 跳过未开始的追加实验，直接汇总 |
| 重复提交同一策略 | 幂等：命中缓存直接返回上次结果 |
